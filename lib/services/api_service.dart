...
class ApiService {
  ...
  Future<String> _getSelectedLanguage() async {
    ...
    return prefs.getString('selectedLanguage') ?? 'English'; // Default to English
  }

  Future<String> sendMessageGPT({required String diseaseName}) async {


    final String selectedLanguage = await _getSelectedLanguage();
              ...
              'content': [
                {
                  'type': 'text',
                  'text':
                      'GPT, your task is to identify plant health issues with precision. Analyze any image of a plant or leaf I provide, and detect all abnormal conditions, whether they are diseases, pests, deficiencies, or decay. Respond strictly in language $selectedLanguage with the name of the condition identified, and nothing else—no explanations, no additional text. If a condition is unrecognizable, reply with \'I don\'t know\'. If the image is not plant-related, say \'Please pick another image\'',
                },
                {
                  'type': 'image_url',
                  'image_url': {
