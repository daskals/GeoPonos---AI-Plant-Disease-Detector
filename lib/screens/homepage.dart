...
import 'package:gpt_vision_leaf_detect/screens/language_selection_screen.dart';
...
class _MyHomePageState extends State<HomePage> {
  ...
  void detectDisease() async {
    setState(() {
      detecting = true;
      diseaseName = ''; // Reset the disease name before new detection
      diseasePrecautions = ''; // Reset the precautions as well
    });

    try {
      String newDiseaseName = await apiService.sendImageToGPT4Vision(image: _selectedImage!);
      setState(() {
        diseaseName = newDiseaseName; // Update the disease name with new detection
      });
    } catch (error) {
        ...
        children: <Widget>[
          ...
          _selectedImage == null
              ? Container(
                  ...
                  child: Image.asset('assets/images/pack2-removebg-preview.png'),
                )
              : Expanded(
                  child: Container(
                    width: double.infinity,
